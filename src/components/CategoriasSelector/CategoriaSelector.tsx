...
interface CategoriasSelectorProps{
  onSelectedCategoria: (categoria: string) => void;
}

const CategoriasSelector: React.FC<CategoriasSelectorProps> = ({onSelectedCategoria}) => {
  const categorias =[
      ...
      <div className="row gap-4">
        {categorias.map((categoria,index)=>(
          <div className="col d-flex justify-content-center p-0" key={index}>
            <button 
            onClick={()=> onSelectedCategoria(categoria.nombre)}
